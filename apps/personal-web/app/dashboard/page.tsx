...
const activityIcon = (type: string) => {
  const map: Record<string, [string, string]> = {
    security: ["bg-amber-100 text-amber-600 dark:bg-amber-500/10 dark:text-amber-400", "M9 12.75 11.25 15 15 9.75m-3-7.036A11.959 11.959 0 0 1 3.598 6 11.99 11.99 0 0 0 3 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285Z"],
    login: ["bg-indigo-100 text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-400", "M15.75 9V5.25A2.25 2.25 0 0 0 13.5 3h-6a2.25 2.25 0 0 0-2.25 2.25v13.5A2.25 2.25 0 0 0 7.5 21h6a2.25 2.25 0 0 0 2.25-2.25V15m3 0 3-3m0 0-3-3m3 3H9"],
    profile: ["bg-emerald-100 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400", "M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z"],
    api: ["bg-purple-100 text-purple-600 dark:bg-purple-500/10 dark:text-purple-400", "M15.75 5.25a3 3 0 0 1 3 3m3 0a6 6 0 0 1-7.029 5.912c-.563-.097-1.159.026-1.563.43L10.5 17.25H8.25v2.25H6v2.25H2.25v-2.818c0-.597.237-1.17.659-1.591l6.499-6.499c.404-.404.527-1 .43-1.563A6 6 0 1 1 21.75 8.25Z"],
  };
  const [cls, path] = map[type] || ["bg-zinc-100 text-zinc-600 dark:bg-zinc-500/10 dark:text-zinc-400", ""];
  return (
        ...
        {stats.map((s, i) => (
          <div key={s.labelKey} className="animate-fade-in rounded-xl border border-zinc-200 bg-white p-5 transition-all hover:border-zinc-300 hover:shadow-sm dark:border-white/[0.06] dark:bg-white/[0.03] dark:backdrop-blur-xl dark:hover:border-white/[0.1] dark:hover:bg-white/[0.05]" style={{ animationDelay: `${i * 60}ms` }}>
            <div className="flex items-center justify-between">
              ...
              {"change" in s && (
                <span className={`rounded-md px-1.5 py-0.5 text-[11px] font-medium ${s.positive ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400" : "bg-red-100 text-red-700 dark:bg-red-500/10 dark:text-red-400"}`}>{s.change}</span>
              )}
            </div>
            ...
            {recentActivity.map((item) => (
              <div key={item.action} className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm text-zinc-800 truncate dark:text-white/80">{item.action}</div>
                  <div className="text-xs text-zinc-500 dark:text-white/30">{item.time}</div>
                </div>
              </div>
            ))}
          </div>
