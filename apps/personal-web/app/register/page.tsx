      ...
      <div className="relative z-10 w-full max-w-sm">
        <div className="relative rounded-2xl border border-zinc-200 bg-white/80 p-8 shadow-lg backdrop-blur-2xl dark:border-white/[0.06] dark:bg-white/[0.03] dark:shadow-2xl dark:shadow-indigo-500/5">
          ...
          <div className="relative">
            ...
            <h1 className="text-2xl font-semibold tracking-tight text-zinc-900 dark:text-white">{t.register_title}</h1>
            <p className="mt-1.5 text-sm text-zinc-500 dark:text-white/35">{t.register_subtitle}</p>
            ...
            <form onSubmit={handleSubmit} className="mt-8 space-y-5">
              ...
              <div>
                ...
                <input id="password" type="password" placeholder={t.password_placeholder} value={password} onChange={(e) => setPassword(e.target.value)} required minLength={6}
                  className="block w-full rounded-xl border border-zinc-200 bg-white px-4 py-2.5 text-sm text-zinc-900 placeholder-zinc-400 outline-none transition-colors focus:border-indigo-400 focus:ring-1 focus:ring-indigo-200 dark:border-white/[0.08] dark:bg-white/[0.04] dark:text-white dark:placeholder-white/20 dark:focus:border-indigo-500/40 dark:focus:bg-white/[0.06]" />
              </div>
              <div>
                <label htmlFor="confirm" className="mb-1.5 block text-xs font-medium text-zinc-500 dark:text-white/45">{t.confirm_label}</label>
                <input id="confirm" type="password" placeholder={t.confirm_placeholder} value={confirm} onChange={(e) => setConfirm(e.target.value)} required
                  className="block w-full rounded-xl border border-zinc-200 bg-white px-4 py-2.5 text-sm text-zinc-900 placeholder-zinc-400 outline-none transition-colors focus:border-indigo-400 focus:ring-1 focus:ring-indigo-200 dark:border-white/[0.08] dark:bg-white/[0.04] dark:text-white dark:placeholder-white/20 dark:focus:border-indigo-500/40 dark:focus:bg-white/[0.06]" />
              </div>
            ...
            <p className="mt-6 text-center text-sm text-zinc-500 dark:text-white/30">
              {t.has_account}{" "}
              <Link href="/login" className="font-medium text-indigo-600 hover:text-indigo-700 dark:text-indigo-400 dark:hover:text-indigo-300">{t.sign_in}</Link>
            </p>
          </div>
        </div>
      </div>
