# 现金流量表 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a cash flow (income & expense) page under `/dashboard/cash-flow` with month navigation, CRUD for income/expense items by category, and summary cards.

**Architecture:** New `"use client"` page in `src/app/dashboard/cash-flow/page.tsx` following the same patterns as the existing balance-sheet page. Sidebar nav item added in `layout.tsx`. All data stored in React useState (in-memory).

**Tech Stack:** Next.js 15, React 19, TypeScript, Tailwind CSS v4, lucide-react

**Design doc:** `docs/superpowers/specs/2026-06-30-cash-flow-design.md`

## Global Constraints

- No external UI/chart libraries — hand-rolled Tailwind only
- No data fetching — all data in React state
- Follow existing balance-sheet patterns (month picker, modals, layout)
- Icons from lucide-react only
- All amounts in CNY (¥), positive numbers only
...
**Files:**
- Modify: `src/app/dashboard/layout.tsx:7` (add import) and `:15` (add nav item)

**Interfaces:**
- Produces: Sidebar item linking to `/dashboard/cash-flow` with `Wallet` icon

- [ ] **Step 1: Add icon import**

Add `Wallet` to the lucide-react import in layout.tsx:
...
Insert after the "资产负债表" entry in `navItems`:
...
Run `npm run build` (or `next build`) and confirm no type/build errors.

- [ ] **Step 4: Commit**

```bash
git add apps/personal-web-admin/src/app/dashboard/layout.tsx
git commit -m "feat: add cash-flow nav item to dashboard sidebar"
```

---

### Task 2: Cash Flow Page — Shell and Month Picker

**Files:**
- Create: `src/app/dashboard/cash-flow/page.tsx`
- Reference: `src/app/dashboard/balance-sheet/page.tsx` (for patterns)

**Interfaces:**
- Produces: Page component with month picker, summary cards area, income/expense sections, add/edit/delete modals
...
- [ ] **Step 2: Add month navigation logic**
...
- [ ] **Step 3: Add computed totals and CRUD handlers**
...
- [ ] **Step 4: Add the month picker JSX**
...
- [ ] **Step 6: Add income/expense sections**
...
- [ ] **Step 7: Add Add/Edit and Delete modals**
...
git commit -m "feat: add cash flow page with monthly income/expense tracking"
```
